...
- If `allChangedFiles` is **empty but `git status --porcelain` shows changes**, archgate's
  ...
  reading each changed file against the ADRs you load from `.archgate/adrs/` (glob
  `.archgate/adrs/*.md`, read each ADR's `domain` frontmatter, and apply those whose
  `files` globs match the changed files).
...
Next: on **APPROVED**, capture learnings with `/lessons-learned`, then commit and open the PR with `/pr`. On **BLOCKED**, return to `/tdd` to fix the cause, then re-review.
