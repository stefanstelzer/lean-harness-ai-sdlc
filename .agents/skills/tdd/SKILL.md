...
For each remaining behaviour: red (next test fails) → green (minimal code passes). One test at a time; only enough code to pass the current test; don't anticipate the next one. Re-run with verbose output to read exactly which assertion fails:
...
Then verify the whole net is green before calling it done:
