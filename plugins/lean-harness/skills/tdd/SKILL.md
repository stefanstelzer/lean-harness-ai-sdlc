...
- List the **behaviours** to test (not implementation steps). **Stop and get the user's explicit approval of this list before writing any test** — do not proceed on your own.
...
Next: review the change with `/reviewer`, capture learnings with `/lessons-learned`, then commit, push, and open the PR with `/pr`.
